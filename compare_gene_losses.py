#!/usr/bin/env python3
"""
compare_gene_losses.py
Align two FASTA sequences (AA or NT), highlight regions *missing in the symbiont*
relative to the free-living reference, and save:
  - <basename>_loss_box_diagram.png/.pdf  (figure)
  - <basename>_loss_segments.csv          (start/end/length of each loss on the reference axis)

Usage (single pair):
  python compare_gene_losses.py --sym rpoH_B_aphidicola.fasta --free rpoH_E_coli.fasta \
      --gene rpoH --sym-label "B. aphidicola" --free-label "E. coli" --out rpoH

You can run this once per pair (see commands at the bottom of this file).
"""
...
def write_outputs(losses, aln_sym, aln_free, free_label, sym_label, gene, outbase):
    ...
    # CSV of loss segments
    df = pd.DataFrame([{f"start_{free_label}": s, f"end_{free_label}": e, "length": e - s + 1}
                       for s, e in losses])
...
def main():
    ...
    ap.add_argument("--gene", required=True, help="Gene name (for titles)")
...
# --------------------------
# rpoH (Buchnera vs E. coli)
# python compare_gene_losses.py --sym rpoH_B_aphidicola.fasta --free rpoH_E_coli.fasta --gene rpoH --sym-label "B. aphidicola" --free-label "E. coli" --out rpoH

# gyrA (C. pneumoniae vs I. pallida)
# python compare_gene_losses.py --sym Q9Z8R4_gyrA.fasta --free E8R6D3_gyrA.fasta --gene gyrA --sym-label "C. pneumoniae" --free-label "I. pallida" --out gyrA

# gltJ (Wigglesworthia vs Salmonella)
# python compare_gene_losses.py --sym gltJ_Wigglesworthia.fasta --free gltJ_Salmonella.fasta --gene gltJ --sym-label "Wigglesworthia" --free-label "Salmonella" --out gltJ

# sucA (Wigglesworthia vs Salmonella fragment)
# python compare_gene_losses.py --sym sucA_Wigglesworthia.fasta --free sucA_Salmonella_fragment.fasta --gene sucA --sym-label "Wigglesworthia" --free-label "Salmonella" --out sucA

# carB (Wigglesworthia vs Salmonella)
# python compare_gene_losses.py --sym carB_Wig.fasta --free carB_Sal.fasta --gene carB --sym-label "Wigglesworthia" --free-label "Salmonella" --out carB
